...
export default function Rus() {
    ...
    const supabaseUrl = "https://nwdkssafheeqfofajdtr.supabase.co";
    const supabaseKey = "sb_publishable_JfMigoy6ai_V2yWD4a-grQ_ZSW_LoQj";
    const supabase = createClient(supabaseUrl, supabaseKey);

    const fetchWord = async (type) => {
        if (type !== "VERB") {
            const minId = Math.floor(Math.random() * (314000 + 1));
            ...
            let pl = (Math.round(Math.random()) + 1) == 1 ? "pl" : "sg";
            let plFlag = false;
            wordFun.forms.forEach((form) => {
                if (form.grammemes.indexOf("pl") !== -1)
                    plFlag = true;
            });
            if (!plFlag) 
                pl = "sg";
            setPlural(pl === "pl");
            let cs = Math.floor(Math.random() * 5) + 1;
            ...
            const ansForm = wordFun.forms.find((form) => {
                return form.grammemes.split(":").indexOf(cs) !== -1 && form.grammemes.split(":").indexOf(pl) !== -1;
            });
            //console.log(wordFun); console.log(pl); console.log(cs); console.log(ansForm);

            let formsToAns = wordFun.forms.filter((f) => {
                return (f.grammemes.split(":").indexOf(cs) === -1 || f.grammemes.split(":").indexOf(pl) === -1) && f.form !== ansForm.form && f.grammemes.split(":").indexOf("V-be") === -1;
            });
    ...
    return (
        <div className="min-h-screen bg-[#0D101C] text-white">
            <div className="flex flex-col items-center justify-center text-center p-8">
                <h2 className="text-4xl font-extrabold mb-4">{l.polish.title}</h2>
                <p className="text-lg mb-8">{l.polish.subtitle}</p>
                ...
                {answered ? (
                    <div>
                        <h1 className="text-xl font-bold p-0.5">{correct ? l.polish.correct : l.polish.incorrect}</h1>
                        <div className="flex gap-3 w-full">
                            ...
                            >
                                {loading ? l.polish.loading : l.polish.nextWord}
                            </button>
                        ...
                        <div className="mt-2 flex flex-col items-center gap-4 ml-4">
                            ...
                            <p className="w-2/3">{l.polish.choose + (plural ? l.plurality.plural : ((wordGend ? wordGend : "") + l.plurality.single)) + wordCase}</p>
                        ...
                        <div className="mt-2 flex flex-col items-start gap-4 ml-4">
                            {page === 1 ? (
                                <div className="flex flex-col items-start">
                                    <h1 className="text-xl font-bold p-0.5">{l.polish.noun}</h1>
                                    ...
                                    >
                                        {loading ? l.polish.loading : l.polish.load}
                                    </button>
                                </div>
                            ) : null}
                            {page === 2 ? (
                                // <div className="flex flex-col items-start">
                                //     <h1 className="text-xl font-bold p-0.5">{l.polish.verb}</h1>
                            ...
                            {page === 3 ? (
                                // <div className="flex flex-col items-start">
                                //     <h1 className="text-xl font-bold p-0.5">{l.polish.adjective}</h1>
